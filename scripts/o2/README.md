...
Create the project-local Python environment through SLURM:
...
If setup fails because of Python package compatibility, capture `logs/` and `results/o2_python_env_check.txt` and do not run smoke/audit yet.
...
* `gpu_audit.sbatch`: loads CUDA, starts the O2 GPU monitor if available, and runs `python -u examples/gpu_backend_audit.py`.
